...
// Per-instance throttle for local/preview safety. Serverless production needs a
// shared store such as Redis/Vercel KV/Upstash to make these limits global.
...
export function rateLimitKey(request, scope, discriminator = '') {
  return `${scope}:${cleanKeyPart(getClientIp(request))}:${cleanKeyPart(discriminator)}`;
}
...
export function enforceEphemeralRateLimit(request, scope, options, discriminator = '') {
